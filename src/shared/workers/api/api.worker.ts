  ...
  if (!storeSubjects[id]) {
    ...
    storeSubjects[id] = {
      ...
      subscribe: (subscriber: (data: unknown) => void) => {
        storeSubjects[id].subscribers.push(subscriber)
        // Return unsubscribe function
        return () => {
          storeSubjects[id].subscribers = storeSubjects[id].subscribers.filter((s) => s !== subscriber)
        }
      },
    }
  }
...
const handleFetchRequestAndUpdateStore = async (
  ...
  try {
    ...
    // Update part of the store with API response
    const currentData = storeSubjects[id].value
    storeSubjects[id].next(responseData)

    // Compare new data with current data
    if (!isEqual(responseData, currentData)) {
      // Send updated part of the store back to the main thread
      postMessage({ id, data: responseData })
    }
  } catch (error: unknown) {
